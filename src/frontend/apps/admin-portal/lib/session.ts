 ...
 */
const ACCESS_TOKEN_KEY = "unify.admin.accessToken";
const REFRESH_TOKEN_KEY = "unify.admin.refreshToken";

/**
 * Fired whenever this tab writes or clears the session. The browser's own "storage" event only
 * fires in OTHER tabs, never the one that made the change, so anything in this tab that needs to
 * react to sign-in/sign-out (AdminNav) needs this instead.
 */
const SESSION_CHANGED_EVENT = "unify:admin-session-changed";
