...
import { isPasswordChangeRequired, isProfileCompletionRequired, readAccessToken } from "@/lib/session";
...
/** Mirrors FORCED_PASSWORD_CHANGE_ROUTE for a Google account missing a phone number. */
export const FORCED_PROFILE_COMPLETION_ROUTE = "/complete-profile";
...
export interface RequireSessionProps {
  ...
  /** Mirrors allowPasswordChangeScope for the mandatory complete-profile screen. */
  allowProfileCompletionScope?: boolean;
}
...
export function RequireSession({
  ...
  allowProfileCompletionScope = false,
}: RequireSessionProps) {
  ...
  const status = !token
    ...
    : isPasswordChangeRequired(token) && !allowPasswordChangeScope
      ...
      : isProfileCompletionRequired(token) && !allowProfileCompletionScope
  ...
  useEffect(() => {
    ...
    } else if (status === "profile-completion-required") {
      router.replace(FORCED_PROFILE_COMPLETION_ROUTE);
    }
  }, [status, router]);
