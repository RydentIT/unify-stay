# UnifyStay admin portal (Next.js standalone).
# Build context is the REPOSITORY ROOT:
#   docker build -f .docker/admin-portal.Dockerfile -t unify-admin-portal .
...
RUN npm run build --workspace=@unify/admin-portal
...
COPY --from=build --chown=nextjs:nodejs /app/apps/admin-portal/.next/standalone ./
COPY --from=build --chown=nextjs:nodejs /app/apps/admin-portal/.next/static ./apps/admin-portal/.next/static
COPY --from=build --chown=nextjs:nodejs /app/apps/admin-portal/public ./apps/admin-portal/public
